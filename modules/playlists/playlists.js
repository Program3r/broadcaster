...
if (Meteor.isClient) {
    ...
    Template.saveload.events({
        'click .save': function () {
            ...
            if (saveval > "") {
                var saveObj = {
                    name: saveval,
                    playlist: playlistpersist.find().fetch()
                };
        ...
        'click .load': function () {
            ...
            var loadObj = savedplaylists.findOne({
                _id: loadid
            });
            ...
            stop: function (event, ui) {
                $("#playlists").find(".exp-item").each(function (index) {
                    ...
                    if (thisItem.hasClass('isotope-item')) {
                        ...
                        playlistpersist.update($(this).attr('id'), {
                            $set: {
                                order: index
                            }
                        });
    ...
    Template.timecontrols.rendered = function () {
        $("#autostarttime").timepicker({
            'timeFormat': 'H:i:s'
        });
    }
    Template.timecontrols.repeat = function () {
    ...
    Template.playlists.events({
        ...
        'click .clearplaylist': function () {
            $.each(playlistpersist.find().fetch(), function () {
                playlistpersist.remove(this._id)
            });
        }
    });
...
if (Meteor.isServer) {
    Meteor.methods({
        ...
        'encode': function (options) {
            console.log('Encode Command Recieved!')
            if (Meteor.isServer) {
        ...
        'encodeNext': function () {
            ...
            Fiber(function () {
                ...
                if (file != undefined) {
                    if (process.env.sim == 'true') {
                        var cmd = "dir -l";
                        console.log('SIMULATED ENCODE');
                    }
                    else {
                        var cmd = "ffmpeg -re -i '" + file.path + "/" + file.filename + "' -vcodec libx264 -ab 128k -ac 2 -ar 44100 -r 25 -s 640x480 -vb 320k -f flv 'rtmp://" + Meteor.settings.rtmpuser + ":" + Meteor.settings.rtmppass + "@" + Meteor.settings.rtmp + "'";
                    }
                    Meteor.call('remcmd', {
                        channel: "ffmpeg",
                        sock: {
